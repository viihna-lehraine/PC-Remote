...
-d)
	...
	umask 077
	sudo -u viihna SOPS_PGP_KEY_ID=$SOPS_PGP_KEY_ID sops --output-type json --decrypt "$ENCRYPTED_KEYFILE" | sudo tee "$KEYFILE" >/dev/null
...
-e)
	...
	sudo -u viihna sops --encrypt --pgp "$SOPS_PGP_KEY_ID" "$KEYFILE" | sudo tee "$ENCRYPTED_KEYFILE" >/dev/null
